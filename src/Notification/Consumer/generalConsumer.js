  ...
  try {
    ...
    await channel.assertQueue(QUEUE_NAME, {
      ...
      arguments: {
        ...
        "x-dead-letter-routing-key": "General.retry.1" // First retry step
      }
    });
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      const retryQueue = `General_Retry_Queue_${i + 1}`;
      const ttl = RETRY_DELAYS[i];
      const nextRetryRoutingKey = (i + 1 < MAX_RETRIES)
        ? `General.retry.${i + 2}`
        : "General.dlq"; // Last retry sends to DLQ
      ...
      await channel.bindQueue(retryQueue, NOTIFICATION_EXCHANGE, `General.retry.${i + 1}`);
    }

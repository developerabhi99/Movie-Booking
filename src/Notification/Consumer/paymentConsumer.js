...
async function PaymentConsumer() {
  try {
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      const queueName = `Payment_Retry_Queue_${i + 1}`;
      ...
      await channel.assertQueue(queueName, {
        ...
        arguments: {
          ...
          "x-dead-letter-routing-key": "Payment.main", // Send back to main queue after TTL
        },
      });
    ...
    await channel.consume(QUEUE_NAME, async (msg) => {
      ...
      try {
        ...
        if (retryCount < MAX_RETRIES) {
          const nextRetryQueue = `Payment_Retry_Queue_${retryCount + 1}`;
          const nextRoutingKey = `Payment.retry.${retryCount + 1}`;
